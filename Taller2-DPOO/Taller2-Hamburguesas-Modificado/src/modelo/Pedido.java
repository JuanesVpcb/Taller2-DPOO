		...
		for (int i = 0; i < this.productos.size(); i++) {
			...
			for (int j = 0; j < factura2.productos.size(); j++) {
				...
				if (eseProd.getClass().equals(ProductoAjustado.class) && ajustado) {
					...
					if (!este.getBase().equals(ese.getBase())) return false;
					if (este.getAdiciones().size() != ese.getAdiciones().size()) return false;
					boolean adicionEncontrada = false;
					for (int k = 0; k < este.getAdiciones().size(); k++) {
						
						Ingrediente ingredienteActual = este.getAdiciones().get(k);
						int freqEste = Collections.frequency(este.getAdiciones(), ingredienteActual);
						int freqEse = Collections.frequency(ese.getAdiciones(), ingredienteActual);
						if (ese.getAdiciones().contains(ingredienteActual) && freqEste == freqEse) adicionEncontrada = true;
						
					}
					if (!adicionEncontrada) return false;
					if (este.getEliminados().size() != ese.getEliminados().size()) return false;
					boolean eliminadoEncontrado = false;
					for (int k = 0; k < este.getEliminados().size(); k++) {
						
						Ingrediente ingredienteActual = este.getEliminados().get(k);
						int freqEste = Collections.frequency(este.getAdiciones(), ingredienteActual);
						int freqEse = Collections.frequency(ese.getAdiciones(), ingredienteActual);
						if (ese.getEliminados().contains(ingredienteActual) && freqEste == freqEse) eliminadoEncontrado = true;
